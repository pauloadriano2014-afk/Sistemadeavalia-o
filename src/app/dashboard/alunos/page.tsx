          ...
          <div className="flex gap-3 w-full md:w-auto">
             <div className="relative flex-1 md:w-64">
                ...
                <input 
                  ...
                  className="w-full bg-zinc-900/50 border border-zinc-800 rounded-xl py-2.5 pl-10 text-white text-sm font-bold focus:border-brand outline-none transition-colors"
                />
             </div>
             <Link href="/dashboard/alunos/novo" className="bg-brand hover:bg-lime-400 text-black px-6 py-2.5 rounded-xl text-sm font-black uppercase tracking-widest flex items-center gap-2 transition-all shadow-[0_0_20px_rgba(132,204,22,0.3)] hover:shadow-[0_0_30px_rgba(132,204,22,0.5)]">
          ...
          {athletes?.map((athlete: any) => {
            ...
            return (
                <Link key={athlete.id} href={`/dashboard/alunos/${athlete.id}`}>
                  <div className="group bg-zinc-900/40 border border-zinc-800 hover:border-brand/50 p-6 rounded-2xl transition-all hover:bg-zinc-900/80 cursor-pointer relative overflow-hidden">
                     
                     <div className="flex items-center gap-4">
                        {/* Avatar / Inicial */}
                        <div className="h-14 w-14 rounded-xl bg-black border border-zinc-800 flex items-center justify-center text-brand font-black text-xl group-hover:scale-110 transition-transform shadow-lg">
                            {athlete.full_name ? athlete.full_name[0].toUpperCase() : '?'}
                        </div>
                        
                        <div className="flex-1 min-w-0">
                            <h3 className="font-bold text-white text-lg truncate group-hover:text-brand transition-colors capitalize">
                              {athlete.full_name}
                            </h3>
                        ...
                        <div className="opacity-0 group-hover:opacity-100 -translate-x-4 group-hover:translate-x-0 transition-all text-brand">
                            <ArrowRight size={24} />
                        </div>
                     </div>
                  </div>
