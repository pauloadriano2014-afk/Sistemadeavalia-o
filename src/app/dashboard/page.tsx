  ...
  return (
    <div className="max-w-7xl mx-auto space-y-10">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-white/10 pb-6">
        <div>
            <div className="flex items-center gap-2 mb-1">
                ...
                <p className="text-brand font-bold text-[10px] uppercase tracking-[0.2em]">Sistema Operacional</p>
            </div>
        ...
        <div className="flex gap-3">
            ...
            <Link href="/dashboard/alunos/novo" className="bg-brand hover:bg-lime-400 text-black px-6 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all shadow-[0_0_20px_rgba(132,204,22,0.3)] hover:shadow-[0_0_30px_rgba(132,204,22,0.5)] flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        {/* Card 1 */}
        <div className="group relative bg-zinc-900/40 backdrop-blur-xl border border-white/5 rounded-3xl p-8 overflow-hidden hover:border-brand/30 transition-all duration-500">
            ...
            <div className="relative z-10">
                ...
                <Link href="/dashboard/alunos" className="mt-6 inline-flex items-center gap-2 text-brand text-xs font-black uppercase tracking-widest hover:text-white transition-colors group-hover:translate-x-2 duration-300">
        ...
        <div className={`group relative backdrop-blur-xl border rounded-3xl p-8 overflow-hidden transition-all duration-500 ${pending > 0 ? 'bg-brand/5 border-brand/20 hover:border-brand/50' : 'bg-zinc-900/40 border-white/5'}`}>
            <div className="absolute top-0 right-0 p-8 opacity-5 group-hover:opacity-10 transition-opacity transform group-hover:scale-110 duration-500">
                <ClipboardList size={120} />
            </div>
            <div className="relative z-10">
                <div className="flex items-center gap-2 mb-4">
                    <div className={`p-2 rounded-lg border text-zinc-400 ${pending > 0 ? 'bg-brand/10 border-brand/20 text-brand' : 'bg-zinc-800/50 border-white/5'}`}>
                        <Activity size={20}/>
                    </div>
                    <span className={`text-[10px] font-black uppercase tracking-widest ${pending > 0 ? 'text-brand' : 'text-zinc-500'}`}>Status do Feed</span>
                </div>
                <div className="flex items-baseline gap-2">
        ...
        <div className="group relative bg-gradient-to-br from-zinc-900/60 to-black border border-white/5 rounded-3xl p-8 overflow-hidden">
            ...
            <div className="flex items-end gap-1 h-24 w-full">
                {[40,60,30,80,50,90,70,100,60,80].map((h, i) => (
                    <div key={i} className="flex-1 bg-zinc-800 hover:bg-brand transition-colors duration-300 rounded-t-sm" style={{height: `${h}%`}}></div>
                ))}
            </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-zinc-900/30 border border-white/5 rounded-3xl p-8 relative overflow-hidden backdrop-blur-sm">
            ...
            <div className="flex justify-between items-center mb-8">
                <h3 className="text-xl font-black text-white italic uppercase tracking-tighter flex items-center gap-3">
                    <Zap className="text-brand" size={24} fill="currentColor"/> Performance da Equipe
                </h3>
                <div className="flex gap-2">
                    <span className="h-3 w-3 bg-brand rounded-full"></span>
            ...
            <div className="h-64 w-full flex items-end justify-between gap-2 md:gap-4 px-2">
                {[35, 50, 45, 60, 80, 70, 90, 100, 85, 60, 75, 95].map((val, i) => (
                    <div key={i} className="w-full flex flex-col justify-end group cursor-pointer">
                        <div 
                            className="w-full bg-zinc-800/50 border-t border-x border-white/5 rounded-t-md group-hover:bg-brand group-hover:shadow-[0_0_20px_rgba(132,204,22,0.4)] transition-all duration-300 relative"
                            style={{ height: `${val}%` }}
                        >
        ...
        <div className="bg-zinc-900/30 border border-white/5 rounded-3xl p-8 backdrop-blur-sm">
            ...
            <div className="space-y-4">
                {recents.map((s: any) => (
                    <div key={s.id} className="flex items-center gap-4 p-4 rounded-2xl bg-black/40 border border-white/5 hover:border-brand/30 transition-all group cursor-pointer">
                        <div className="h-10 w-10 rounded-xl bg-zinc-900 flex items-center justify-center text-brand font-black border border-white/5 group-hover:scale-110 transition-transform">
